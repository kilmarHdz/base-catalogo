...
<template>
  <div>
    <button
      @click="toggleDrawer"
      :aria-label="count > 0 ? `Ver carrito, ${count} artículos` : 'Ver carrito'"
      :aria-expanded="isOpen"
      class="fixed bottom-6 right-6 z-50 w-14 h-14 bg-brand-accent hover:bg-brand-accent/90 text-white rounded-full flex items-center justify-center shadow-xl hover:shadow-2xl transition-all duration-300 transform motion-safe:hover:scale-105 active:scale-95 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent/60"
    >
      ...
      <span
        v-if="count > 0"
        aria-live="polite"
        class="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] font-black w-6 h-6 rounded-full flex items-center justify-center border-2 border-brand-background shadow-md motion-safe:animate-bounce"
      >
        {{ count }}
      </span>
    </button>
    ...
    >
      <div class="p-6 border-b border-brand-border flex items-center justify-between">
        ...
        <button
          ...
          class="p-2 rounded-lg border border-brand-border text-brand-secondary hover:bg-brand-background hover:text-brand-primary transition-colors duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent/50"
        >
        ...
        <ul v-else class="space-y-4 divide-y divide-brand-border/40" role="list">
          ...
          >
            ...
            <div class="flex-grow space-y-1">
              ...
              <div class="flex items-center gap-2 pt-1.5">
                <button
                  ...
                  class="w-6 h-6 rounded-md border border-brand-border bg-brand-background text-brand-secondary hover:bg-brand-border/40 flex items-center justify-center text-xs font-black transition-colors duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent/50"
                >
                  −
                </button>
                <span
            ...
            <button
              ...
              class="p-2 rounded-lg text-brand-secondary hover:text-red-500 hover:bg-red-500/10 transition-all duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/50"
            >
      ...
      <div v-if="items.length > 0" class="p-6 border-t border-brand-border bg-brand-background/40 space-y-4">
        ...
        <div class="flex flex-col gap-3">
          <button
            @click="checkout"
            class="w-full py-4 rounded-xl bg-brand-accent hover:bg-brand-accent/90 text-white font-bold text-sm shadow-lg shadow-brand-accent/25 hover:shadow-xl transition-all duration-300 transform active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent/60"
          >
          ...
          <button
            @click="clear"
            class="w-full py-2.5 rounded-xl border border-brand-border hover:bg-red-500/10 hover:border-red-500/30 text-brand-secondary hover:text-red-500 text-xs font-bold transition-all duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/40"
          >
            Vaciar Carrito
          </button>
        </div>
    ...
    <div class="fixed top-24 right-6 z-50 flex flex-col gap-3 w-full max-w-sm pointer-events-none" role="status" aria-live="polite">
